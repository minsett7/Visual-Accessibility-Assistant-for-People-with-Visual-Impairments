...
# client = Groq(api_key="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")
# MODEL = "llama-3.1-8b-instant"  # Groq model id

# # Simple safety gate: only allow humor when no hazards in scene
# def humor_allowed(scene):
#     for o in scene.get("objects", []):
#         if o.get("label") == "car" and o.get("motion") == "moving" and o.get("distance_m", 999) <= 6:
#             return False
#         if o.get("label") in {"stairs"}:
#             return False
#         if o.get("label") == "traffic_light" and o.get("state") == "red":
#             return False
#     return True

# def build_messages(scene_json, question, kb):
#     tone = "friendly, concise; tiny gentle joke allowed" if humor_allowed(scene_json) else "calm, safety-first; no humor"
#     system = (
#         "You are a mobility assistant for a blind user. Keep answers to 1–2 short sentences for TTS. "
#         "Never invent objects not present in the Scene JSON. If unsure, say what you need. "
#         f"Tone: {tone}."
#     )
#     user = (
...
# def ask(scene_json, question, kb):
#     resp = client.chat.completions.create(
#         model=MODEL,
#         messages=build_messages(scene_json, question, kb),
#         temperature=0.5,
#         max_tokens=120,
#     )
